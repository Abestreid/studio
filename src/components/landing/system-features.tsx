...
const features = [
  {
    icon: <i className="ph ph-magnifying-glass"></i>,
  ...
  {
    icon: <i className="ph ph-identification-card"></i>,
  ...
  {
    icon: <i className="ph ph-funnel"></i>,
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => (
            <Card key={feature.title} className={`p-6 flex flex-col items-start gap-4 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 hover:scale-102.5 ${feature.gradient ? `bg-gradient-to-br ${feature.gradient}` : 'bg-white'}`}>
                <div className={`rounded-full p-3 w-12 h-12 flex items-center justify-center ${feature.gradient ? 'bg-accent text-white' : 'bg-accent/10 text-accent'} text-2xl mb-3`}>
                  {feature.icon}
                </div>
                <div>
